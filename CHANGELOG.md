...
+ [新增] 渠道兼容Gemini格式。
+ [新增] 新增Codex App插件支持。
+ [新增] 新增内置 `/api/ai` 代理，支持通过服务器环境变量接入 APIB/APIMart 的图片、视频和模型接口。
+ [新增] 画布上传素材支持一次选择多张图片、视频或音频，并在画布中自动错位创建多个节点。
+ [新增] 画布支持从剪切板直接粘贴图片到当前画布，并自动创建图片节点。
+ [修复] 修复 APIB/APIMart 图生图请求失败问题，将图片编辑上传适配为任务式 `image_urls` 提交。
+ [修复] 修复提示词库在服务器无法访问 raw.githubusercontent.com 时加载失败，以及部分 HTML 图片封面无法提取的问题。
+ [修复] 修复图生图提示词中 `@` 只能在文本开头触发素材引用的问题，现在可在任意文字位置触发。
+ [修复] 修复使用 IP/HTTP 访问画布时本地 Agent ID 生成和自动连接导致页面报错的问题。
+ [优化] Docker 构建改为直接调用 Next.js 构建入口，减少运行镜像构建阶段的外部 apt 依赖。
